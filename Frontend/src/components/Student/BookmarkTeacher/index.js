import classNames from "classnames/bind";
import styles from "./BookmarkTeacher.module.scss";
import { FaStar } from "react-icons/fa";
const cx = classNames.bind(styles);

function BookmarkTeacher() {
  return (
    <div className={cx("wrapper")}>
      <div className={cx("title")}>
        <h1>これはあなたの仮申し込みました教師です。</h1>
      </div>
      <div className={cx("box-bookmark")}>
        <div className={cx("left")}>
          <img className={cx('img-bookmark')} src="data:image/jpeg;base64,"></img>
        </div>
        <div className={cx("center")}>
          <h2>Nguyen Anh Tu</h2>
          <div className={cx("pd-rating")}>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
          </div>
          <div className={cx("description")}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin
            ultrices posuere hendrerit. Aenean vitae fringilla justo, vitae
            iaculis lacus. Suspendisse euismod, magna et molestie congue, sapien
            mi aliquet ipsum, non fringilla urna sem non justo. Suspendisse a
            ultricies elit.
          </div>
        </div>
        <div className={cx("right")}>
          <div>
            <button className={cx('button-bookmark')}>もっと詳しく</button>
          </div>
          <div>
            <button className={cx('button-bookmark')}>削除</button>
          </div>
        </div>
      </div>
      <div className={cx("box-bookmark")}>
        <div className={cx("left")}>
          <img className={cx('img-bookmark')} src="data:image/jpeg;base64,"></img>
        </div>
        <div className={cx("center")}>
          <h2>Nguyen Anh Tu</h2>
          <div className={cx("pd-rating")}>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
          </div>
          <div className={cx("description")}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin
            ultrices posuere hendrerit. Aenean vitae fringilla justo, vitae
            iaculis lacus. Suspendisse euismod, magna et molestie congue, sapien
            mi aliquet ipsum, non fringilla urna sem non justo. Suspendisse a
            ultricies elit.
          </div>
        </div>
        <div className={cx("right")}>
          <div>
            <button className={cx('button-bookmark')}>もっと詳しく</button>
          </div>
          <div>
            <button className={cx('button-bookmark')}>削除</button>
          </div>
        </div>
      </div>
      <div className={cx("box-bookmark")}>
        <div className={cx("left")}>
          <img className={cx('img-bookmark')} src="data:image/jpeg;base64,"></img>
        </div>
        <div className={cx("center")}>
          <h2>Nguyen Anh Tu</h2>
          <div className={cx("pd-rating")}>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
            <i>
              <FaStar />
            </i>
          </div>
          <div className={cx("description")}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin
            ultrices posuere hendrerit. Aenean vitae fringilla justo, vitae
            iaculis lacus. Suspendisse euismod, magna et molestie congue, sapien
            mi aliquet ipsum, non fringilla urna sem non justo. Suspendisse a
            ultricies elit.
          </div>
        </div>
        <div className={cx("right")}>
          <div>
            <button className={cx('button-bookmark')}>もっと詳しく</button>
          </div>
          <div>
            <button className={cx('button-bookmark')}>削除</button>
          </div>
        </div>
      </div>
    </div>
  );
}
export default BookmarkTeacher;
